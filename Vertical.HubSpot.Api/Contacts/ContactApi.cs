...
namespace Vertical.HubSpot.Api.Contacts {
    ...
    public class ContactApi : IContactApi {
        ...
        IEnumerable<Parameter> GetListParameters(long? offset, int? limit, params string[] properties)
        {
        ...
        public async Task<PageResponse<T>> ListPage<T>(long? offset = null, int? limit = null, params string[] properties)
            where T:HubSpotContact
        {
            EntityModel model = models.Get(typeof(T));

            JObject response = await rest.Get<JObject>("contacts/v1/lists/all/contacts/all", GetListParameters(offset, limit ?? 100, properties).ToArray());
